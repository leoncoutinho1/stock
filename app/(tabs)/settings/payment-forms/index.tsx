...
import { PaymentForm, deletePaymentForm, listPaymentForms } from '@/src/api/paymentForm';
import { Ionicons } from '@expo/vector-icons';
import { useRouter } from 'expo-router';
import { useEffect, useState } from 'react';
import { ActivityIndicator, Alert, FlatList, StyleSheet, Text, TouchableOpacity, View } from 'react-native';

export default function PaymentFormsScreen() {
    ...
    const loadPaymentForms = async () => {
        try {
            const result = await listPaymentForms();
    ...
    const handleDelete = (id: string, description: string) => {
        Alert.alert(
            'Confirmar exclusão',
            `Deseja realmente excluir a forma de pagamento "${description}"?`,
            [
    ...
    const renderItem = ({ item }: { item: PaymentForm }) => (
        <View style={[styles.card, { backgroundColor: cardBg, borderColor }]}>
            <TouchableOpacity
                style={styles.cardContent}
                onPress={() => router.push(`/settings/payment-forms/${item.id}` as any)}
            >
    ...
    return (
        <View style={[styles.container, { backgroundColor: bgColor }]}>
            <View style={styles.header}>
                ...
                <Text style={[styles.title, { color: textColor }]}>Formas de Pagamento</Text>
            </View>
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={styles.emptyContainer}>
                        <Ionicons name="card-outline" size={64} color={textColor} style={{ opacity: 0.3 }} />
            ...
            <TouchableOpacity
                style={styles.fab}
                onPress={() => router.push('/settings/payment-forms/new' as any)}
            >
